...
function main() {
  ...
  //    Fallos no bloquean el informe (template degrada con gracia) pero
  //    se loguean a stderr para diagnóstico — no silencio completo.
  let insights = { cards: [], derived: null, goals: null };
  if (mode === 'week') {
    try {
      const metricsFile = path.join(METRICS_DIR, `${data.period}.json`);
      if (!fs.existsSync(metricsFile)) {
        console.error(`[insights] metrics file no existe: ${metricsFile}`);
      } else {
      ...
      console.error(`[insights] error generando insights: ${e.message}`);
    }
  }
