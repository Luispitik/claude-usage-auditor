#!/usr/bin/env node
/**
 * NextGen AI Institute Productivity · aggregate
 *
 * Lee los .jsonl de ~/.nextgenai-productivity/events/
 * y genera métricas agregadas por:
 *   - sesión (session_id)
 *   - día (YYYY-MM-DD)
 *   - semana (YYYY-Www, ISO)
 *
 * Uso:
 *   node aggregate.js week              → semana actual
 *   node aggregate.js week 2026-W16     → semana concreta
 *   node aggregate.js week last         → semana pasada
 *   node aggregate.js day               → hoy
 *   node aggregate.js day 2026-04-16    → día concreto
 *
 * Output: imprime JSON por stdout + escribe ~/.nextgenai-productivity/metrics/{period}.json
 */
...
function isoWeekOf(date) {
  // ISO week: thursday del mismo year define el año ISO
...
function weekRange(weekLabel) {
  // weekLabel = "2026-W16" → devuelve [mondayDate, sundayDate] en UTC
  const m = weekLabel.match(/^(\d{4})-W(\d{2})$/);
  if (!m) throw new Error(`Formato de semana inválido: ${weekLabel}`);
...
function parseDateArg(arg, today) {
  ...
  throw new Error(`Formato de fecha inválido: ${arg}`);
}
...
function parseWeekArg(arg, today) {
  ...
  throw new Error(`Formato de semana inválido: ${arg}`);
}

// -------- lectura de eventos --------
function loadEventsForDates(dateList) {
  const events = [];
...
function emptySessionMetrics() {
  return {
    ...
    tool_calls: 0,
    tool_errors: 0,
    tools: {},          // { Edit: 12, Bash: 5, ... }
    cwds: {},           // { "C:/proj-a": 3, ... } - proyectos tocados en la sesión
    input_bytes: 0,
    output_bytes: 0,
    retries: 0,         // mismo hash en <60s
    denials: 0          // tool denegado por usuario (is_error con mensaje "denied")
  };
}
...
function aggregateBySession(events) {
  ...
  // index para detectar retries: { "sessionId|tool|hash": [timestamps] }
  ...
  for (const ev of events) {
    ...
    s.end_ts = ev.ts; // siempre el último timestamp

    if (ev.type === 'tool_start') {
      s.tool_calls++;
      s.tools[ev.tool] = (s.tools[ev.tool] || 0) + 1;
      if (ev.cwd) s.cwds[ev.cwd] = (s.cwds[ev.cwd] || 0) + 1;
      if (ev.input_size) s.input_bytes += ev.input_size;

      // detector de retries
      if (ev.input_hash) {
        const key = `${ev.session_id}|${ev.tool}|${ev.input_hash}`;
        const prev = hashIndex.get(key) || [];
        const tsMs = new Date(ev.ts).getTime();
        if (prev.some(prevTs => tsMs - prevTs < 60_000 && tsMs - prevTs > 0)) {
          s.retries++;
        }
        prev.push(tsMs);
      ...
      if (ev.ok === false) s.tool_errors++;
    }
  }

  // calcular duración por sesión. Guardia defensiva: si el cálculo es NaN,
  // negativo o > 24h (timestamps corruptos / clock skew), trunca a 0 para
  // evitar que metrics derivadas propaguen basura.
...
function main() {
  ...
  if (mode === 'day') {
    ...
    console.error(`Modo desconocido: ${mode}. Usa 'day' o 'week'.`);
    process.exit(1);
  }

  const events = loadEventsForDates(dateList);
  ...
  // output para consumo de render.js
  process.stdout.write(JSON.stringify(result));
}

if (require.main === module) main();

module.exports = { isoWeekOf, weekRange, datesInRange, aggregateBySession, aggregateByDay, aggregateByWeek };
